...
local function getCachedClosest()
    ...
    for _, animalData in ipairs(allAnimalsCache) do
        ...
        if animalPos then
            ...
            if dist < CONFIG.STEAL_RANGE and dist < bestDist then
                bestDist = dist
                best = animalData
            end
        end
...
local function executeStealAsync(prompt, animalData, myGeneration)
    ...
    task.spawn(function()
        ...
        -- ENTRY DELAY: Using Heartbeat timer (same as scan interval)
        entryDelayTimer = 0
        while entryDelayTimer < 0.016 do  -- ~1 frame at 60fps
            if myGeneration ~= StealGeneration then
                data.ready = true
                return
            end
...
local function attemptSteal(target)
    -- COOLDOWN: Using Heartbeat timer (same as scan interval)
    if cooldownTimer < 0.016 then  -- ~1 frame at 60fps
        return false
    end
...
local function createUI()
    ...
    card.Name = "Card"
    card.Size = UDim2.new(0, 240, 0, 130)
    ...
    S.UserInputService.InputChanged:Connect(function(input)
        ...
        if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
            ...
            local newY = math.clamp(dragCard.startPos.Y.Offset + delta.Y, 0, workspace.CurrentCamera.ViewportSize.Y - 130)
    ...
    -- Row 2: Show Progress Bar Toggle
    ...
    lbl2.Size = UDim2.new(0, 140, 1, 0)
    lbl2.BackgroundTransparency = 1
    lbl2.Text = "SHOW PROGRESS BAR"
